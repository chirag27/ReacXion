...
//Global Variables
//================================================================
char frame_data[64];	//Frame Data buffer contains 64 Bytes which correspond to the color for each LED on the RGB Matrix. This Frame will be sent over SPI.
char character_leds[8];	//The Character LED buffer contains the color for one row of LEDs on the RGB Matrix. These values are copied into the Frame Data buffer.

char text_color=RED, background_color=BLACK;	//These values define what color will be filled in the Frame Data buffer. Colors are defined in Setup.h

//Messages to be displayed on the Matrix
char message_1[50]="WELCOME";
char message_2[50]="TO";
char message_3[50]="SPARKFUN!";
...
void ioinit(void)
{
	...
	UBRR0H = (MYUBRR) >> 8;			//Set the Baud Rate in the UBRR register
	UBRR0L = MYUBRR;
	UCSR0A = (1<<U2X0);				//Use double clock frequency
	UCSR0B = (1<<RXEN0)|(1<<TXEN0);	//Enable Rx and Tx
	UCSR0C = (3<<UCSZ00);				//Set character size to 8 bits
...
//Usage:	letter=uart_getchar();
//Purpose:	Gets a character from the UART
//Inputs:	None
//Outputs:	Returns a character if it has been input on the Rx line; else returns 0.
...
//Usage:	letter=wait4_char()
//Purpose:	Waits for a character to be input to the UART
//Inputs:	None
//Outputs:	Returns a character that has been put onto the UART
...
//Inputs:	uint16_t x->amount of time to delay
//Outputs:	None
...
void putchar_frame(char *message, int position, char color){
	...
	if(position >= 0){
		...
		for(int row=position; row<8; row++){
	...
	//then fill the remaining rows with the next character.
		...
		for(int row=-8; row<position; row++){
